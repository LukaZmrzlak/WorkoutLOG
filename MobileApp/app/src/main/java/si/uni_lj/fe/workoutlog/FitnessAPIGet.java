...
public class FitnessAPIGet implements Callable<String> {
    private final String urlService, token, date;
    private final Activity callerActivity;

    public FitnessAPIGet(String token, String date, String urlService, Activity callerActivity) {
        this.token = token;
    ...
    public String JSONString2String(String JsonString){
        ...
        try {
            ...
            for(int i=0; i<jsonArray.length(); i++){
                Output += jsonArray.getJSONObject(i).getString("vaja");
                Output += ": ";
                Output += jsonArray.getJSONObject(i).getString("seti");
